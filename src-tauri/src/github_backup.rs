...
pub async fn github_list_repos(token: String) -> Result<Vec<GhRepo>, String> {
    ...
    for r in arr {
        if let Some(name) = r.get("full_name").and_then(|v| v.as_str()) {
            out.push(GhRepo { full_name: name.to_string() });
        }
    }
    Ok(out)
}
...
// 上传最新备份到仓库
#[tauri::command]
pub async fn github_upload_backup(token: String, repo: String, data_dir: String) -> Result<String, String> {
    ...
    // 先看是否已存在（需要 sha 才能更新）
    let sha = match cli.get(&url).send().await {
    ...
    if resp.status().is_success() {
        ...
        let code = resp.status().as_u16();
        Err(format!("上传失败，GitHub 返回 {}（仓库名对吗？需 owner/name 格式）", code))
    }
}

// 从仓库下载备份到本地 backups 目录
#[tauri::command]
pub async fn github_download_backup(token: String, repo: String, data_dir: String) -> Result<String, String> {
    let backup_dir = Path::new(&data_dir).join("backups");
    ...
    let bytes = B64.decode(clean).map_err(|e| format!("解码失败：{}", e))?;
