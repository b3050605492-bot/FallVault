...
- ☁️ **GitHub 私有仓库备份** - 设置页「GitHub 备份」标签：粘贴 Personal Access Token（PAT）→ 选私有仓库 → 一键上传 / 下载加密的 `.fvault`。**仅同步已加密文件，主密码绝不上传**（详见下方「GitHub 备份说明」）
...
1. GitHub 右上角头像 → **Settings → Developer settings → Personal access tokens** → 生成一个 token（classic），勾选 `repo` 权限
2. 复制生成的令牌（以 `ghp_` 或 `github_pat_` 开头），粘贴到 FallVault
3. 点 **「获取我的仓库」** → 在下方下拉里选一个**私有仓库**
4. **「上传备份」** 把本地最新的加密 `.fvault` 推到仓库根目录的 `fallvault-backup.fvault`；**「下载备份」** 拉回本地（之后在「加密备份」里恢复）

> ⚠️ **安全提醒**：同步的只是**已经用主密码加密好的 `.fvault` 文件**；**主密码本身绝不上传**，也不会以明文存到仓库。令牌（PAT）仅保存在本软件内存中，不写入磁盘、不上传。忘记主密码请依靠你自己的记忆 / 提示，不要寄望于把主密码明文塞进仓库——那等于把万能钥匙公开。
...
- [x] GitHub 私有仓库备份（PAT 上传/下载加密 .fvault，主密码绝不上传）
