...
### 动态验证码（TOTP / OTP）
- ⏱️ **TOTP 动态码** - 自动生成验证码（默认 6 位 / SHA1 / 30 秒，兼容 **SHA256/SHA512、8 位、自定义周期**），30 秒倒计时 + 进度条，一键复制
- 🔗 **密钥导入** - 直接粘贴 `otpauth://` URI、原始 Base32 密钥，或**谷歌验证器批量迁移链接**（`otpauth-migration://`，自动识别算法/位数/周期，新建账号自动提取第一条，设置里可批量导入多条）
- 📲 **Google 验证器批量迁移** - 支持解析 `otpauth-migration://` 批量导入（正确还原 secret 与算法参数）
- ⏰ **时间偏移校正** - 若本机时间与验证器设备不一致导致验证码不符，可在「设置 → 基础 → TOTP 时间偏移」填入差值（秒）对齐
...
- 🌐 **多语言** - 中文 / English 实时切换
- 🔒 **免验证时长 & 锁屏** - 关闭窗口最小化到托盘并锁定（未开启免验证时长时，重新打开需再次输入主密码）；可在设置中开启「免验证时长」，解锁后若干分钟内重新打开/从托盘唤起免输密码（1/5/15/30/60 分钟可选，超期自动回锁）
- 🖥️ **启动即锁定 + 系统托盘** - 关闭窗口最小化到托盘，从托盘重新打开按免验证策略决定是否需输入主密码
- 🔒 **单实例运行** - 多次双击 exe 只打开一个应用，重复启动会聚焦已存在的窗口
- 📤 **多格式完整导出** - 账号导出为 Excel (.xlsx) / 文本 (.txt) / CSV (.csv) / JSON (.json)；包含账号、密码、网站、2FA(TOTP)密钥、分类、标签、自定义字段、密码历史、附件列表、备注、收藏、时间等全部字段；并自动解密导出**附件真文件**到 `{文件名}_attachments/` 子目录（按账号分子文件夹，便于迁移/备份）
- ⌨️ **快速打开热键** - 可在设置中绑定一个全局快捷键（默认 Ctrl+Q），软件在后台/托盘/最小化时一键唤起主窗口；支持单键或组合键（Ctrl/Alt/Shift + 主键），可清除关闭
...
桌面版与 iPhone 版 **FallVault** 使用同一套 `.fvault` 加密备份格式，可双向恢复：

- **桌面版 → iOS**：桌面版导出备份 → iOS 版「恢复备份」直接可读，账号 / 验证码 / 标签完整还原
- **iOS → 桌面版**：iOS 版导出备份 → 桌面版「恢复备份」直接可读，自动去重不重复添加
- **银行卡数据**：手机版独有的银行卡信息会随备份带到桌面版；桌面版没有卡片界面，但会**原样保管**，下次导出备份时带回去 —— 所以「iOS → 桌面版 → iOS」这一圈**不会丢卡片**

> iPhone 版仓库：[FallVault-iOS](https://github.com/b3050605492-bot/FallVault-iOS)
...
### 📝 近期更新（v1.1.x）
- v1.1.8 快速打开热键：设置中可绑定一个全局快捷键（默认 Ctrl+Q），软件在后台/托盘/最小化时一键唤起主窗口；支持单键或组合键（Ctrl/Alt/Shift + 主键），可清除关闭。快捷键在 Rust 端注册，窗口隐藏到托盘时依然有效...[truncated]
- v1.1.6 免验证时长：删除闲置自动锁定，改为「免验证时长」开关 + 时长选择（1/5/15/30/60 分钟）。开启后解锁若干分钟内重新打开/托盘唤起免输密码，超期自动回锁；未开启时关闭到托盘再开仍强制重输密码（权限交给用户）
- v1.1.5 TOTP 修复：谷歌验证器迁移链接正确解析算法/位数/周期（SHA1/SHA256/SHA512、6/8 位、自定义周期）；修复 base32 末尾截断导致 secret 错误；新增「TOTP 时间偏移校正」（设置→基础，补偿本机与验证器设备时间差）
