...
### 📝 近期更新（v1.1.x）
- v1.1.7 导出补全：xlsx/csv/txt/json 四种格式均包含全部字段（账号/密码/网站/2FA TOTP 密钥/分类/标签/自定义字段/密码历史/附件列表/备注/收藏/时间）；新增 CSV 导出；导出时自动解密并复制**附件真文件**到 `{文件名}_attachments/` 子目录（按账号分子文件夹），便于只靠本软件拿验证码的用户迁移备份
